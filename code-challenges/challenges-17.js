...
const recursionPattern = (int1, int2) => {
    if (int1 <= 0) {
        ...
        let arr = []
        arr.push(int1)
        arr.push(recursionPattern(int1 - int2, int2));
        arr.push(int1)
        // flat creates a new array with all the elements of the subarrays concatenated to 
        // it recursively up to a specified depth.
        return arr.flat()
    }
}
// -------------------------------------------------------------------------------------------------------
...
const isPalindrome = (str) => {
    var regex = /[:,\s]/g;
    var string = str.replace(regex, '').toLowerCase();
